...
def __main__(*args):
    ...
    script_dialog.AddControlToGrid("NameLabel", "LabelControl", "Batch Name", 1, 0,
        "Optional: Enter a name for your batch. If left blank, it will default to 'Untitled'.", False)
    ...
    script_dialog.AddSelectionControlToGrid("IncludeTimestamp", "CheckBoxControl", True, "Include Timestamp", 1, 2,
        "Add the current date and time to the batch name for easier identification.")

    script_dialog.AddControlToGrid("CommentLabel", "LabelControl", "Comment", 2, 0,
        "Optional: Add a description for this batch. Applied to all jobs in the batch.", False)
    ...
    script_dialog.AddControlToGrid("DepartmentLabel", "LabelControl", "Department", 3, 0,
        "Optional: Specify your department. Applied to all jobs in the batch.", False)
    ...
    script_dialog.AddControlToGrid("PoolLabel", "LabelControl", "Pool", 1, 0, "The primary pool for your job.", False)
    ...
    script_dialog.AddControlToGrid("SecondaryPoolLabel", "LabelControl", "Secondary Pool", 2, 0,
        "Optional: Fallback pool if the primary pool has no available Workers.", False)
    script_dialog.AddControlToGrid("SecondaryPoolBox", "SecondaryPoolComboControl", "", 2, 1)

    script_dialog.AddControlToGrid("GroupLabel", "LabelControl", "Group", 3, 0, "The group to submit your job to.", False)
    ...
    script_dialog.AddControlToGrid("PriorityLabel", "LabelControl", "Priority", 4, 0,
        "Set a numeric priority for the job.", False)
    ...
    script_dialog.AddControlToGrid("TaskTimeoutLabel", "LabelControl", "Task Timeout", 5, 0,
        "Maximum minutes a Worker can spend on a task before it is requeued. 0 = no limit.", False)
    ...
    script_dialog.AddSelectionControlToGrid("AutoTimeoutBox", "CheckBoxControl", False, "Enable Auto Task Timeout", 5, 2,
        "Automatically calculate task timeout based on previous frame render times if configured in Repository Options.")

    script_dialog.AddControlToGrid("ConcurrentTasksLabel", "LabelControl", "Concurrent Tasks", 6, 0,
        "Number of tasks a Worker can process simultaneously.", False)
    ...
    script_dialog.AddControlToGrid("MachineLimitLabel", "LabelControl", "Machine Limit", 7, 0,
        "Maximum number of machines rendering this job concurrently. 0 = no limit.", False)
    ...
    script_dialog.AddControlToGrid("MachineListLabel", "LabelControl", "Machine List", 8, 0,
        "List of allowed or denied machines, depending on the selection above.", False)
    script_dialog.AddControlToGrid("MachineListBox", "MachineListControl", "", 8, 1, colSpan=2)

    script_dialog.AddControlToGrid("LimitGroupLabel", "LabelControl", "Limits", 9, 0,
        "Specify limits required by this job.", False)
    ...
    script_dialog.AddControlToGrid("DependencyLabel", "LabelControl", "Dependencies", 10, 0,
        "Specify existing jobs this job depends on.", False)
    script_dialog.AddControlToGrid("DependencyBox", "DependencyControl", "", 10, 1, colSpan=2)

    script_dialog.AddControlToGrid("OnJobCompleteLabel", "LabelControl", "On Job Complete", 11, 0,
        "Action to perform after job completion (archive/delete/etc.).", False)
    ...
    script_dialog.AddControlToGrid("BlendFileLabel", "LabelControl", "Blender File", 1, 0,
        "Path to the .blend file to render.", False)
    script_dialog.AddSelectionControlToGrid("BlendFileBox", "FileBrowserControl", "", "Blender Files (*.blend);;All Files (*)", 1, 1, colSpan=2)

    script_dialog.AddControlToGrid("ChunkSizeLabel", "LabelControl", "Frames Per Task", 4, 0, "Number of frames rendered per task.", False)
    script_dialog.AddRangeControlToGrid("ChunkSizeBox", "RangeControl", 1, 1, 1000000, 0, 1, 4, 1, expand=False)
    script_dialog.AddSelectionControlToGrid("SubmitBlendFileBox", "CheckBoxControl", False, "Include Blender File With Job", 4, 2)
...
def _submit_button_pressed(*args):
    ...
    if not File.Exists(scene_file):
        ...
        result = script_dialog.ShowMessageBox("The Blender file %s is saved on a local drive. Are you sure you want to continue?" % scene_file, "Warning", ("Yes","No"))
        if result == "No":
            return
    
    if states_file is None:
        script_dialog.ShowMessageBox("Batch submission via EasyStates is only available through the Blender interface. Please use the 'Submit to Deadline' button in the EasyStates Render panel.", "Error")
        return
    if not File.Exists(states_file):
        script_dialog.ShowMessageBox("The EasyStates scene states .txt file %s does not exist" % states_file, "Error")
        return
